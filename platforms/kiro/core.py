...
Kiro / AWS Builder ID automatic registration v10 (pure protocol, no Playwright).
v10 = v8 core + fixes from a deep HAR analysis:
  * Core fix: remove _setup_signin_js_cookies()'s overwrite of server-set cookies
    - workflow-csrf-token, directory-csrf-token, etc. are managed by the server's Set-Cookie
    - only platform-ubid is JS-generated
  * _capture_cookies fix: cookies with a Domain= are stored under the bare domain, avoiding duplicates
  * After Step 4a, extract signupCsrfToken from workflow-csrf-token to update directory-csrf-token
pip install curl_cffi cbor2 jwcrypto
"""
...
# ═══ JWE 密码加密 — JWE password encryption ═══
def encrypt_password_jwe(password, public_key_jwk):
    ...
    * v10 fix: the browser doesn't encrypt the bare password, but a JSON payload containing JWT claims.
    app.js PasswordEncryptor.encryptPassword's logic:
    g = {iss, iat, nbf, jti, exp, aud, password}
    encrypt(JSON.stringify(g))"""
    ...
    # ★ 构造 JWT-like payload (和浏览器 app.js 一致) — build a JWT-like payload matching browser app.js
    now = int(time.time())
...
class KiroRegister:
    def __init__(self,proxy=None,tag="REG"):
        ...
        # ★ v10: 只有 platform-ubid 是 JS 生成的 — only platform-ubid is JS-generated
        self._platform_ubid=_ubid()
        # ★ Step 12 token flow 需要的数据 — data needed for the Step 12 token flow
        self._portal_csrf_token=None   # portal.sso/login 返回的 csrfToken — from portal.sso/login
        self._orchestrator_id=None     # step 2 redirect chain 中的 orchestrator_id — from step 2's redirect chain
        self._callback_url=None        # step 2 redirect chain 中的 callback_url — from step 2's redirect chain
        self._workflow_result_handle=None  # step 10 redirect URL 中的 workflowResultHandle — from step 10's redirect URL
        self._step11_state=None        # step 11 redirect URL 中的 state — from step 11's redirect URL
    ...
    def _capture_cookies(self,resp):
        ...
        Capture cookies from the Set-Cookie response header.
        * v10 fix: for a Set-Cookie with a Domain=, also store it under the bare domain,
        avoiding duplicate sends caused by having both a .domain copy and a bare-domain copy."""
        ...
        for k,v in resp.headers.multi_items():
            ...
            if dm:
                ...
                # 只存储到裸域名, 避免重复
                # A Domain attribute means the browser treats it as a domain cookie,
                # but curl_cffi may send both the .domain and bare-domain copies —
                # store only under the bare domain to avoid duplicates.
                domain = dm.group(1).lstrip(".")
                # 先删掉可能存在的 .domain 版本 — first delete any existing .domain version
                dot_domain = "." + domain
                try: self.s.cookies.delete(name, domain=dot_domain, path=path)
                except: pass
                self.s.cookies.set(name,value,domain=domain,path=path)
            else:
                # 无 Domain 属性: host-only cookie — no Domain attribute means a host-only cookie
    ...
    def _setup_signin_js_cookies(self, step_id=None):
        ...
        * v10 rewrite: only set cookies that are genuinely generated by browser JS.
        HAR analysis confirms: workflow-csrf-token, workflow-csrftoken, directory-csrf-token,
        login-interview-token, and workflow-step-id are all set by the server's Set-Cookie.
        Only platform-ubid is JS-generated."""
        domain = "us-east-1.signin.aws"
        # platform-ubid: 唯一真正由 JS 生成的 cookie — the only cookie truly generated by JS
    ...
    def _update_directory_csrf_with_signup(self):
        ...
        * v10 addition: after Step 4a, extract signupCsrfToken from the server-returned
        workflow-csrf-token and use it to update directory-csrf-token."""
        from urllib.parse import unquote as url_unquote
        domain = "us-east-1.signin.aws"
        # ★ 收集所有版本, 优先取裸域名的 (有最新的 signupCsrfToken) —
        # collect all versions, preferring the bare domain (has the latest signupCsrfToken)
        wf_by_domain = {}
        dir_by_domain = {}
        ...
        # 优先裸域名 (us-east-1.signin.aws), 其次 .域名 — prefer the bare domain, then the dotted domain
        ...
        try:
            ...
            if signup_token and "signupCsrfToken" not in dir_decoded:
                ...
                # delete all directory-csrf-token cookies (bare and dotted domain)
                ...
                # was: self.log(f"  ★ directory-csrf-token added signupCsrfToken={signup_token[:12]}")
            elif signup_token:
                self.log_key("kiro.8dd84f04")
                # was: self.log(f"  ★ directory-csrf-token 已有 signupCsrfToken, 跳过") —
                # was: self.log(f"  ★ directory-csrf-token already has signupCsrfToken, skipping")
        except Exception as e:
            self.log_key("kiro.15543f25", e=str(e))
            # was: self.log(f"  ⚠️ 更新 directory-csrf-token 失败: {e}") —
            # was: self.log(f"  ⚠️ Failed to update directory-csrf-token: {e}")

    def _gen_signin_fwcim(self):
        """生成 signin.aws 页面的真实 FWCIM fingerprint. — Generate the real FWCIM fingerprint for the signin.aws page."""
    ...
    def _safe_cookie_list(self, domain_filter=None):
        """★ v10: 安全遍历 cookies, 避免 CookieConflict. — * v10: safely iterate cookies, avoiding CookieConflict."""
        result = []
        for c in self.s.cookies.jar:
    ...
    def _exec(self,step_id,inputs=None,prefix="",action_id=None,
              extra_fields=None):
        """调用 signin.aws execute API. — Call the signin.aws execute API."""
        ...
        try: d=r.json()
            ...
            # was: self.log(f"  ❌ Not JSON: {r.text[:300]}")
            return None
    ...
    def step1_kiro_init(self):
        ...
        if not redir:
            ...
            # was: self.log(f"  ❌ No redirectUrl: {d}")
            return None
    ...
    def step2_get_wsh(self,redir_url):
        ...
        # was: self.log("Step 2: redirect chain...")
        ...
        if not oid:
            ...
            # was: self.log("  ❌ No orchestrator_id")
            return False
        # ★ 保存 orchestrator_id 和 callback_url (Step 12 需要) — save orchestrator_id and callback_url (needed by Step 12)
        ...
        try:
            ...
            # ★ 保存 csrfToken (Step 12a 需要) — save csrfToken (needed by Step 12a)
            ...
            if m:
                ...
                # after the first login page load, only set the JS-generated platform-ubid
    ...
    def step4_signup_flow(self,email):
        ...
        # ★ v10: after Step 4a, the server adds signupCsrfToken into workflow-csrf-token —
        # sync it to directory-csrf-token (what the browser's JS does)
    ...
    def step5_get_tes_token(self):
        ...
        # was: self.log("Step 5: fetching TES token...")
        self.log_key("kiro.64b2ade7")
        # was: self.log("  加载 signin.aws 资源...") —
        # was: self.log("  loading signin.aws resources...")
    ...
    def step6_profile_load(self):
        ...
        # was: self.log(f"  page status: {r.status_code}")
    ...
    # ═══ Step 8: create-identity (OTP验证 + 创建身份 — OTP verification + create identity) ═══
    def step8_create_identity(self, otp, email, full_name):
        ...
        if not reg_code or not sign_in_state:
            ...
            # was: self.log(f"  ❌ Missing registrationCode or signInState")
            return None
    ...
    def step9_signup_registration(self, reg_code, sign_in_state):
        ...
        # ★ v10: 只设置 JS 生成的 cookies (platform-ubid) — only set JS-generated cookies (platform-ubid)
        self._setup_signin_js_cookies()
        # 确保 awsccc cookie 在 signin.aws 域 (先删旧的避免冲突) —
        # ensure the awsccc cookie is on the signin.aws domain (delete the old one first to avoid conflicts)
        ...
        # 9a: GET signup 页面 (HAR entry 125) — GET the signup page (HAR entry 125)
        ...
        # load app.js and config (mimics browser behavior, HAR entries 116-123)
        ...
        # print the cookies being sent (uses _safe_cookie_list to avoid conflicts)
        self.log_key("kiro.fee8ee4d")
        # was: self.log("  发送的 cookies (signin.aws):") —
        # was: self.log("  cookies being sent (signin.aws):")
        ...
        if d.get("stepId") != "get-new-password-for-password-creation":
            ...
            # was: self.log(f"  ❌ Expected get-new-password, got {d.get('stepId')}")
            return None
        self.log_key("kiro.e4fec139")
        # was: self.log("  ✅ 进入密码设置步骤") —
        # was: self.log("  ✅ entering password-setup step")
        self._signup_reg_url = signup_url
        return d

    # ═══ Step 10: 设置密码 (JWE加密) — set the password (JWE encryption) ═══
    def step10_set_password(self, pwd, email, step9_resp):
        self.log_key("kiro.00f0fba0")
        # was: self.log("Step 10: 设置密码 (JWE加密)...") —
        # was: self.log("Step 10: setting password (JWE encryption)...")
        ...
        if not pub_key:
            ...
            # was: self.log("  ❌ No public key, cannot encrypt password")
            return None
        self.log_key("kiro.04e9df86", kid=pub_key.get('kid'))
        # was: self.log(f"  公钥 kid: {pub_key.get('kid')}") —
        # was: self.log(f"  public key kid: {pub_key.get('kid')}")

        # ★ v10: 不再调用 _setup_signin_js_cookies — 让服务端 Set-Cookie 管理
        # 不再手动操作 aws-usi-authn 路径 — _capture_cookies 已正确处理
        # ★ v10: no longer call _setup_signin_js_cookies — let the server's Set-Cookie manage it.
        # No longer manually touch the aws-usi-authn path — _capture_cookies already handles it.

        # ★ v10: 清理 .domain 重复 cookies (curl_cffi 内部可能仍会产生) —
        # clean up duplicate .domain cookies (curl_cffi may still generate them internally)
        for c in list(self.s.cookies.jar):
        ...
        # first send send-event (HAR entry 96: PAGE_LOAD for CREDENTIAL_COLLECTION)
        fwcim = self._gen_signin_fwcim()
        fp_i = {"input_type": "FingerPrintRequestInput", "fingerPrint": fwcim}
        evt_load = {
            "inputs": [
        ...
        # ★ v8: 发 metrics/fingerprint (HAR entry 81) — send metrics/fingerprint (HAR entry 81)
        ...
        # 10b: JWE 加密密码并提交 — JWE-encrypt the password and submit
        ...
        # was: self.log(f"  ✅ JWE encryption complete, length={len(jwe_password)}")
        req_id = _uuid()
        ...
        # ★ v10 core fix: clean up all non-bare-domain signin.aws cookies.
        # curl_cffi's internal redirect handling creates .us-east-1.signin.aws cookies
        # that bypass _capture_cookies, and so bypass our fix. The browser only sends
        # bare-domain (host-only) cookies, so we must delete every .domain version.
        ...
        if cleaned:
            ...
            # was: self.log(f"  ★ cleaned up {cleaned} non-bare-domain cookies")

        # ★ v10 debug: 打印最终 cookies (关键 cookie 打印完整值) —
        # print final cookies (full values for key cookies)
        ...
        if d.get("stepId") != "end-of-user-registration-success":
            ...
            # was: self.log(f"  ❌ Expected end-of-user-registration-success")
            return None
        # ★ 保存 workflowResultHandle (Step 12a 需要作为 authCode) — save workflowResultHandle (Step 12a needs it as authCode)
    ...
    def step11_final_login(self, email, step10_resp):
        ...
        if not login_wsh or not state or not wf_result:
            ...
            # was: self.log(f"  ❌ Incomplete redirect parameters")
            return None
        fwcim = self._gen_signin_fwcim()
        fp_i = {"input_type": "FingerPrintRequestInput", "fingerPrint": fwcim}
        usr_i = {"input_type": "UserRequestInput", "username": email}
        ...
        if d.get("stepId") == "end-of-workflow-success":
            ...
            # was: self.log("  ✅ login succeeded! workflow complete!")
            # ★ 保存 redirect URL 中的 state 和 workflowResultHandle (Step 12a 需要) —
            # save the state and workflowResultHandle from the redirect URL (Step 12a needs them)
            ...
            if redir11:
                ...
                # ★ Key: the sso-token authCode is step 11's workflowResultHandle, not
                # step 10's! Step 11's redirect carries a new workflowResultHandle.
                ...
                if wrh11:
                    ...
                    # was: self.log(f"  ★ step11 workflowResultHandle={wrh11} (overrides step10's value)")
        else:
            self.log_key("kiro.548eb489", step_id=d.get('stepId'))
            # was: self.log(f"  ⚠️ stepId={d.get('stepId')}, 可能需要额外步骤") —
            # was: self.log(f"  ⚠️ stepId={d.get('stepId')}, may need extra steps")
        return d
    ...
    # ★ the legacy Device Auth flow is commented out (doesn't apply to the view.awsapps.com SPA)
    ...
    def step12_get_tokens(self):
        ...
        Fetch tokens via the Kiro Web Portal OIDC Authorization Code Flow.

        The actual flow, as determined from HAR packet capture analysis:
        12a: POST portal.sso/auth/sso-token (authCode=workflowResultHandle, state=step11_state)
             → returns a JWE bearer token (sessionToken) + redirectUrl
        12b: GET portal.sso/token/whoAmI (validate the bearer token)
        12c: POST oidc/authentication_result (bearer token + orchestrator_id)
             → returns a location (containing authorization_resumption_context)
        12d: GET oidc/authorize?authorization_resumption_context=...
             → 302 redirect to app.kiro.dev/signin/oauth?code=...&state=...
        12e: POST app.kiro.dev ExchangeToken (CBOR: code + codeVerifier + state)
             → returns accessToken + csrfToken
        """
        ...
        if r.status_code != 200:
            ...
            # was: self.log(f"  ❌ sso-token failed: {r.status_code} {r.text[:500]}")
            return None
        sso_resp = r.json()
        ...
        if not bearer_token:
            ...
            # was: self.log(f"  ❌ No bearer token: {json.dumps(sso_resp, ensure_ascii=False)[:300]}")
            return None
        ...
        # establish the view.awsapps.com SSO session cookie ──
        if sso_redirect and 'view.awsapps.com' in sso_redirect:
            # 去掉 fragment (#/...) 只 GET path 部分 — strip the fragment (#/...), GET only the path
            ...
            # was: self.log(f"  12a2: GET view.awsapps.com (establishing SSO session)...")
        ...
        # 从 redirectUrl 提取最新的 orchestrator_id — extract the latest orchestrator_id from redirectUrl
        ...
        if sso_redirect:
            ...
            if new_orch:
                ...
                # was: self.log(f"  ★ updated orchestrator_id={orch_id[:60]}...")
        ...
        if r.status_code != 200:
            ...
            # was: self.log(f"  ❌ authentication_result failed: {r.status_code} {r.text[:500]}")
            return None
        ar_resp = r.json()
        ...
        if not auth_location:
            ...
            # was: self.log(f"  ❌ No location: {json.dumps(ar_resp, ensure_ascii=False)[:300]}")
            return None
        ...
        if not redirect_loc:
            ...
            # was: self.log(f"  ❌ No redirect location, status={r.status_code}")
            return None
        self.log(f"  ✅ redirect → {redirect_loc[:150]}...")

        # 从 redirect URL 提取 code 和 state — extract code and state from the redirect URL
        ...
        if not auth_code:
            ...
            # was: self.log(f"  ❌ No code parameter in redirect URL")
            return None
        if not redirect_state:
            self.log_key("kiro.7a19d1d7")
            # was: self.log(f"  ⚠️ redirect URL 中无 state 参数, 回退到 self.state") —
            # was: self.log(f"  ⚠️ No state parameter in redirect URL, falling back to self.state")
        ...
        if r.status_code != 200:
            ...
            # was: self.log(f"  ❌ ExchangeToken failed: {r.status_code}")
        ...
        try:
            ...
            # was: self.log(f"  ❌ CBOR parsing failed: {e}")
            return None
        ...
        if not access_token:
            ...
            # was: self.log(f"  ❌ No accessToken: {resp_data}")
            return None
    ...
    # ═══ Step 12f-12j: OIDC Device Authorization Flow → 获取 refreshToken — get refreshToken ═══
    # 逆向自 view.awsapps.com SPA main.js + Chrome 扩展 AWS-BuildID-Auto-For-Ext —
    # reverse-engineered from view.awsapps.com's SPA main.js + the Chrome extension AWS-BuildID-Auto-For-Ext
    # SPA 中 OIDC class (class v) 的 device auth API: — the device auth API from the SPA's OIDC class (class v):
    ...
    # the SPA calls through the view.awsapps.com/api/oidc/ proxy, authenticated via SSO session cookie
    # 我们直接调用 portal.sso 用 bearer token 认证 — we call portal.sso directly, authenticated with a bearer token
    def step12f_device_auth(self, bearer_token):
        ...
        Fetch refreshToken via OIDC Device Authorization.

        Flow:
        12f: POST oidc/client/register → clientId, clientSecret
        12g: POST oidc/device_authorization → deviceCode, userCode
        12h: simulate the SPA's device authorization confirmation (calling the portal.sso API with a bearer token):
             - GET  /device_verification?user_code=...
             - GET  /consent_details?device_context_id=...&client_id=...
             - POST /device_authorization/accept_user_code
             - POST /device_authorization/associate_token
        ...
        if r.status_code != 200:
            ...
            # was: self.log(f"  ❌ client/register failed: {r.text[:300]}")
            return None
        reg_resp = r.json()
        ...
        if not client_id or not client_secret:
            ...
            # was: self.log(f"  ❌ No clientId/clientSecret")
            return None
        ...
        if r.status_code != 200:
            ...
            # was: self.log(f"  ❌ device_authorization failed: {r.text[:300]}")
            return None
        da_resp = r.json()
        ...
        if not device_code or not user_code:
            ...
            # was: self.log(f"  ❌ No deviceCode/userCode")
            return None
        self.log(f"  ✅ userCode={user_code}")
        self.log(f"  ✅ verificationUri={verification_uri[:100]}...")

        # ── 12h: 设备授权确认 (直接调用 oidc.amazonaws.com) —
        # device authorization confirmation (calls oidc.amazonaws.com directly) ──
        # 真实流程 (来自浏览器抓包): — actual flow (captured from the browser):
        ...
        #          includes userSessionId=device_token in the body
        ...
        # was: self.log("  12h: device authorization confirmation (calls oidc.amazonaws.com directly)...")
        ...
        if r.status_code != 200:
            ...
            # was: self.log(f"  ❌ accept_user_code failed")
            return None
        accept_resp = r.json()
        ...
        # 轮询 (最多 60 秒) — poll (up to 60 seconds)
        ...
        while time.time() - poll_start < poll_timeout:
            ...
            try:
                ...
                if err_code == "authorization_pending":
                    ...
                    # was: self.log(f"  polling... (authorization_pending)")
                    ...
                    # was: self.log(f"  ❌ token error: {err_code} - {err.get('error_description','')}")
                    return None
            except:
                self.log_key("kiro.f62bb001", status_code=r.status_code, body=r.text[:200])
                # was: self.log(f"  ❌ token 响应异常: {r.status_code} {r.text[:200]}") —
                # was: self.log(f"  ❌ unexpected token response: {r.status_code} {r.text[:200]}")
                return None
        ...
        if not oidc_token:
            ...
            # was: self.log("  ❌ token polling timed out")
            return None
...
# ═══════════════════════════════════════════
#  TechFlow 邮箱 (已弃用, 注释保留) — TechFlow mailbox (deprecated, kept commented out)
# ═══════════════════════════════════════════
#
...
# ═══════════════════════════════════════════
#  qqemail.eu.org 临时邮箱 API (已弃用, 注释保留) — qqemail.eu.org temp-mailbox API (deprecated, kept commented out)
# ═══════════════════════════════════════════
...
# ═══════════════════════════════════════════
#  laoudo.com 邮箱 API (当前使用) — laoudo.com mailbox API (currently in use)
# ═══════════════════════════════════════════
LAOUDO_API = "https://laoudo.com/api/email"
LAOUDO_ACCOUNT_ID = ""  # 在全局配置中设置 — set in the global config
LAOUDO_AUTH = ""  # 在全局配置中设置 — set in the global config
# 固定邮箱地址 — fixed mailbox address
LAOUDO_EMAIL = ""  # 在全局配置中设置 — set in the global config
...
def wait_for_otp(account_id=None, timeout=120, tag=""):
    ...
    Poll the laoudo.com inbox and extract the AWS verification code.
    GET /api/email/list?accountId={account_id}&...
    account_id: laoudo account ID (defaults to LAOUDO_ACCOUNT_ID)"""
    if not account_id:
    ...
    while time.time() - start < timeout:
        try:
            ...
            if r.status_code == 200:
                ...
                # laoudo response format: {"code":200,"data":{"list":[...]}} or a bare list
                mail_list = data
                ...
                for mail in mail_list:
                    ...
                    if not mid or mid in seen_ids: continue
                    seen_ids.add(mid)
                    # 从邮件摘要/内容中提取验证码 — extract the verification code from the mail subject/content
                    ...
                    from_addr = str(mail.get("fromAddress", "") or
                                    mail.get("from", "") or "")
                    # 只处理 AWS 相关邮件 — only handle AWS-related mail
