...
def suppress_insecure_request_warning():
    ...
    Suppress urllib3's TLS warning only when certificate verification is explicitly disabled."""
...
def insecure_request(request_callable, *args, **kwargs) -> Any:
    ...
    Perform a requests call with verify=False and suppress the corresponding warning."""
...
def mark_session_insecure(session: Any) -> Any:
    ...
    Mark a requests.Session as verify=False; pair it at the call site with
    suppress_insecure_request_warning.
    Returns the session to allow chaining.
    """
