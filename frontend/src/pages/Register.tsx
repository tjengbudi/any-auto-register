  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_340px]">
        <div className="space-y-4">
          <Card>
            <CardHeader><CardTitle>{catalog.register.basicConfigTitle}</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <Select label={catalog.register.platformLabel} k="platform" options={platformOptions} />
              <div className="grid gap-4 md:grid-cols-2">
                <Input label={catalog.register.batchCountLabel} k="count" type="number" />
                <Input label={catalog.register.proxyOptionalLabel} k="proxy" placeholder="http://user:pass@host:port" />
              </div>
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle>{catalog.register.stepOneTitle}</CardTitle></CardHeader>
          ...
          <Card>
            <CardHeader><CardTitle>{catalog.register.stepTwoTitle}</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              ...
              {form.identity_provider === 'oauth_browser' && (
                <>
                  <Input label={catalog.register.oauthEmailHintLabel} k="oauth_email_hint" placeholder="your-account@example.com" />
                  <Input label={catalog.register.chromeProfilePathLabel} k="chrome_user_data_dir" placeholder="~/Library/Application Support/Google/Chrome" />
                  <Input label={catalog.register.chromeCdpUrlLabel} k="chrome_cdp_url" placeholder="http://localhost:9222" />
                  <p className="text-xs text-[var(--text-muted)]">
                    {catalog.register.oauthBrowserHint}
                  </p>
                </>
              )}
            </CardContent>
          ...
          {form.identity_provider === 'mailbox' && (
            <Card>
              <CardHeader><CardTitle>{catalog.register.mailboxConfigTitle}</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {hasOptionsError && (
                  <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300">
                    {catalog.register.optionsErrorLoadFailed}
                  </div>
                )}
                {mailboxProviderOptions.length > 0 ? (
                  <Select label={catalog.register.mailboxProviderLabel} k="mail_provider" options={mailboxProviderOptions} />
                ) : (
                  <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
                    {catalog.register.mailboxProviderMissing}
                  </div>
                )}
          ...
          {smsProviderOptions.length > 0 && (
            <Card>
              <CardHeader><CardTitle>{catalog.register.smsConfigTitle}</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {hasOptionsError && (
                  <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300">
                    {catalog.register.optionsErrorLoadFailed}
                ...
                <Select label={catalog.register.smsProviderLabel} k="sms_provider" options={smsProviderOptions} />
        ...
        <div className="space-y-5 xl:sticky xl:top-4 xl:self-start">
          <Card className="bg-[var(--bg-pane)]/62">
            <CardHeader>
              <CardTitle>{catalog.register.summaryTitle}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Button onClick={submit} disabled={polling} className="w-full">
                {polling ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />{catalog.register.submittingLabel}</> : <><Play className="mr-2 h-4 w-4" />{catalog.register.startRegisterLabel}</>}
              </Button>
            </CardContent>
          </Card>
          ...
          {task ? (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="rounded-lg border border-[var(--border-soft)] bg-[var(--chip-bg)] p-3 text-xs text-[var(--text-secondary)]">
                    <div>{catalog.register.taskIdLabel}</div>
                  ...
                  {task.status === 'interrupted' && !task.error && (
                    <div className="flex items-center gap-2 text-amber-400">
                      <XCircle className="h-4 w-4" />
                      <span className="text-xs">{catalog.register.taskInterruptedMessage}</span>
                    </div>
                  )}
                  {task.status === 'cancelled' && !task.error && (
                    <div className="flex items-center gap-2 text-amber-400">
                      <XCircle className="h-4 w-4" />
                      <span className="text-xs">{catalog.register.taskCancelledMessage}</span>
                    </div>
                  )}
                </CardContent>
              </Card>
              <Card>
                <CardHeader><CardTitle>{catalog.register.liveLogTitle}</CardTitle></CardHeader>
                <CardContent>
            ...
            <Card className="bg-[var(--bg-pane)]/55">
              <CardHeader><CardTitle>{catalog.register.waitingTitle}</CardTitle></CardHeader>
              <CardContent className="text-sm text-[var(--text-secondary)]">{catalog.register.waitingDescription}</CardContent>
            </Card>
          )}
        </div>
      </div>
