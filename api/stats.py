"""注册成功率仪表盘 API。 — Registration success-rate dashboard API."""
...
def stats_by_platform():
    """按平台统计成功率。 — Success rate broken down by platform."""
    with Session(engine) as session:
        rows = session.exec(
            select(
                TaskLog.platform,
...
def stats_by_day(days: int = 30, platform: str = ""):
    """按天统计注册趋势。 — Registration trend broken down by day."""
...
@router.get("/by-proxy")
def stats_by_proxy():
    """代理成功率排行。 — Proxy success-rate ranking."""
    with Session(engine) as session:
...
def stats_errors(days: int = 7, platform: str = "", limit: int = 20):
    """最近的失败错误聚合（按错误信息分组）。 — Recent failure errors aggregated (grouped by error message)."""
    cutoff = _utcnow() - timedelta(days=days)
    with Session(engine) as session:
        q = (
